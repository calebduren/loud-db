...
export function EditReleaseForm({ release, onSuccess }: EditReleaseFormProps) {
  ...
  const updateArtist = (index: number, value: string) => {
    ...
    const existingArtist = artists.find(a => a.name.toLowerCase() === value.toLowerCase());
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow-md">
      ...
      <ReleaseFormFields
        ...
        onAddArtist={() => setSelectedArtists([...selectedArtists, { name: '' }])}
        onRemoveArtist={(index) => setSelectedArtists(selectedArtists.filter((_, i) => i !== index))}
      />
