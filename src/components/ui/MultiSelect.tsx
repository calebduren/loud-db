...
export function MultiSelect({
  ...
  return (
    <div className={cn("relative", className)} ref={ref}>
          ...
          {value.length === 0 ? (
            ...
            value.map(v => {
              ...
              return option ? (
                <span
                  key={v}
                  className="bg-white/10 text-sm px-2 py-0.5 rounded-full flex items-center gap-1"
                >
                  {option.label}
                  <span
                    role="button"
      ...
      {open && (
        <div className="absolute z-10 w-full mt-1 bg-background border border-white/10 rounded-md shadow-lg">
          <div className="max-h-60 overflow-auto">
            {options.map(option => (
              <div
                key={option.value}
                className={cn(
                  "flex items-center px-2 py-1.5 cursor-pointer hover:bg-white/10",
